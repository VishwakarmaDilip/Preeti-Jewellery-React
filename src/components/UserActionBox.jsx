...
const UserActionBox = ({ onClose }) => {
  ...
  return (
    <div className="fixed xs:absolute z-30 bg-white justify-between flex flex-col top-0 -right-0 xs:-right-36 p-2 w-full h-full xs:h-fit xs:w-72 border shadow-md rounded-md">
      <div>
      ...
      {/* Log Out */}
      <div className="flex justify-between items-center xs:items-stretch flex-col pt-2 border-t-2">
        <button
          onClick={() => handleLogout()}
          className="bg-red-500 xs:bg-white rounded-xl text-white xs:text-black flex justify-between p-2 xs:hover:bg-gray-100"
        >
          <div className="flex items-center gap-3">
            <Icon.LogOut />
            <p className="font-semibold">Logout</p>
          </div>
          <Icon.ChevronRight className="hidden xs:inline-block" />
        </button>
      </div>
    </div>
  );
